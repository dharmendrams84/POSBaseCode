 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/admin/ejournal/SalesLog.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:07 mszekely Exp $
 * ===========================================================================
 * NOTES
    ...
    {                                   // begin writeTransactionTotals()
        ...
        {                               // begin write tax details
            ...
            {                           // begin write tax data
                ...
                {
                    // tax groups info in totals deprecated in 7.0
//                    writeTaxGroups(tt.getDefaultRate(),
//                                   totals.getTaxGroups());
                }
                writeBuf("");
            }                           // end write tax data
    ...
    **/
    ...
    /**
        ...
        @param defaultRate default transaction tax rate
        @param vtg Vector of Tax Group objects
    **/
    //---------------------------------------------------------------------
    protected void writeTaxGroups(double defaultRate, Vector vtg)
    {                                   // begin writeTaxGroups()
        Enumeration e = vtg.elements();
        DecimalFormat df = new DecimalFormat("#0.0000%");
        StringBuffer sb;
        String amountString;
        CurrencyIfc exceptionAmounts = DomainGateway.getBaseCurrencyInstance();
        while (e.hasMoreElements())
        {                               // begin loop through tax groups
            TaxGroup tg = (TaxGroup) e.nextElement();
            sb = new StringBuffer("  ");
            switch (tg.getGroupType())
            {                           // begin evaluate tax group type
                case TaxGroup.TAX_GROUP_BY_RATE:
                    // if rate same as default
                    if (tg.getRate() == defaultRate)
                    {
                        sb.append("Tax ");
                        amountString = df.format(tg.getRate());
                        sb.append(pad(TAX_RATE_LENGTH,
                                      amountString.length(),
                                      0));
                        sb.append(amountString);
                        sb.append("   ");
                        amountString = tg.getGroupTaxTotal().toFormattedString();
                        sb.append(pad(EXTENDED_PRICE_LENGTH,
                                      amountString.length(),
                                      0));
                        sb.append(amountString);
                    ...
                    // if transaction override
                    else
                    {
                        // roll into exception amounts
                        exceptionAmounts =
                          exceptionAmounts.add(tg.getGroupTaxTotal());
                    }
                    break;
                case TaxGroup.TAX_GROUP_BY_TRANSACTION_OVERRIDE_AMOUNT:
                case TaxGroup.TAX_GROUP_BY_ITEM_OVERRIDE_AMOUNT:
                default:
                    // roll into exception amounts
                    exceptionAmounts =
                      exceptionAmounts.add(tg.getGroupTaxTotal());
                    break;
            }                           // end evaluate tax group type
        }                               // end loop through tax groups
        // if any exceptions, write them here
        if (exceptionAmounts.signum() == CurrencyIfc.POSITIVE)
        {                               // begin write exceptions amount
            sb = new StringBuffer("  Tax Exceptions ");
            amountString = exceptionAmounts.toFormattedString();
            sb.append(pad(EXTENDED_PRICE_LENGTH,
                          amountString.length(),
                          0));
            sb.append(amountString);
            writeBuf(sb.toString());
        }                               // end write exceptions amount
    }                                   // end writeTaxGroups()
    ...
    {                                   // begin writeTenderAmounts()
        ...
        {                               // begin loop through tender lines
            ...
            {                           // begin write credit card data
                ...
                int len = tli.getNumber().length();
                sb.append(tli.getNumber().substring(len - 4, len));
                sb.append("  Exp ");
