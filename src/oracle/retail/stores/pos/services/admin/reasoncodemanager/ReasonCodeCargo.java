 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/admin/reasoncodemanager/ReasonCodeCargo.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:08 mszekely Exp $
 * ===========================================================================
 * NOTES
...
public class ReasonCodeCargo extends UserAccessCargo implements CargoIfc, TourCamIfc, DBErrorCargoIfc
{
    ...
    /** The store (e.g., 2006) currently selected for reason code
        setting.  **/
    ...
    /**
     * code list map for update
     **/
    protected CodeListMapIfc codeListMapForUpdate = null;
    ...
    // ----------------------------------------------------------------------
    /**
     * Returns the code list map for updating.
     * <P>
     * 
     * @return The code list map for updating.
     * @deprecated as of 13.1. No callers
     **/
    // ----------------------------------------------------------------------
    public CodeListMapIfc getCodeListMapForUpdate()
    {
        return codeListMapForUpdate;
    }

    // ----------------------------------------------------------------------
    /**
     * Sets the code list map for updating.
     * <P>
     * 
     * @param value The code list map for updating.
     * @deprecated as of 13.1. No callers
     **/
    // ----------------------------------------------------------------------
    public void setCodeListMapForUpdate(CodeListMapIfc value)
    {
        codeListMapForUpdate = value;
    }
     ...
     **/
    // --------------------------------------------------------------------------
    public ReasonCodeGroupBeanModel convertCodeListToBean(CodeListIfc list)
    {
        ...
        group.setDefaultReasonCode(list.getDefaultOrEmptyString());
        group.setStoreID(list.getStoreID());

        // Create a reason code from group
        Vector reasonCodes = new Vector();
        Vector deletedCodes = new Vector();
        CodeEntryIfc[] entries = list.getEntries();
        for (int i = 0; i < entries.length; i++)
        {
            ReasonCode reasonCode = new ReasonCode();
            reasonCode.setReasonCodeName(entries[i].getText());
            if (entries[i].getCodeString().equals(CodeConstantsIfc.CODE_UNDEFINED))
            {
                reasonCode.setDatabaseId("0");
            }
            else
            {
                reasonCode.setDatabaseId(entries[i].getCodeString());
            }
        ...
        {
            reasonCodes.addElement(enumer.nextElement());
        }
    ...
    public CodeListIfc convertBeanToCodeList(ReasonCodeGroupBeanModel rcGroup)
    {
        ...
        for (int i = 0; i < beanValues.size(); i++)
        {
            ...
            if ((bean.getReasonCodeName() != null)
                && bean.getReasonCodeName().equals(rcGroup.getDefaultReasonCode()))
            {
     ...
     * <B>Post-Condition</B>
     * <P>
     * All fields are returned to their initial values (existing values are
     * erased).
     **/
    // --------------------------------------------------------------------------
    public void reset()
