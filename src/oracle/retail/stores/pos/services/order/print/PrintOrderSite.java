 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/order/print/PrintOrderSite.java /main/17 2011/02/16 09:13:28 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class PrintOrderSite extends PosSiteActionAdapter
{
    ...
    protected void updateStatus(OrderIfc order)
    {
        ...
        int orderType=0;
        if(order.getOriginalTransaction() instanceof OrderTransaction)
        {
           orderTransaction=(OrderTransaction)order.getOriginalTransaction();
        ...
        if (orderType == OrderConstantsIfc.ORDER_TYPE_ON_HAND)
        {
            for (int i = 0; i < orderItems.length; i++)
            {
                ...
                if (itemStatus == OrderLineItemIfc.ORDER_ITEM_STATUS_NEW)
                {
                    ((SaleReturnLineItemIfc)orderItems[i]).getOrderItemStatus().getStatus().changeStatus(
                            OrderLineItemIfc.ORDER_ITEM_STATUS_PENDING);
                    newItemCnt++;
                }
                else if (itemStatus == OrderLineItemIfc.ORDER_ITEM_STATUS_PICKED_UP
                        && ((SaleReturnLineItemIfc)orderItems[i]).getOrderItemStatus().getItemDispositionCode() == OrderLineItemIfc.ORDER_ITEM_DISPOSITION_SALE)
                {
                    ((SaleReturnLineItemIfc)orderItems[i]).getOrderItemStatus().getStatus().changeStatus(
