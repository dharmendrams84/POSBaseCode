 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/order/common/AutomaticEmailSite.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:33 mszekely Exp $
 * ===========================================================================
 * NOTES
    ...
    **/
    ...
    public void arrive(BusIfc bus)
    {
        ...
        String emailAddress =
            cargo.getOrder()
                 .getCustomer()
                 .getEmailAddress(EmailAddressConstantsIfc.EMAIL_ADDRESS_TYPE_HOME).getEmailAddress();
        ...
        if (emailAddress.length() > 0)
        {
            ...
            String      callingService  = cargo.getServiceName();
            ...
            switch (orderStatus)
            {
                ...
                case OrderConstantsIfc.ORDER_STATUS_FILLED:
                    if (callingService.equals(OrderCargoIfc.SERVICE_PICKUP))
                    {
                        try
                        {
            ...
            if (sendEmail) //attempt to send the message
            {
                ...
                AbstractTransactionLineItemIfc[] allItems = null;
                int itemsLength = 0;
                if (callingService.equals(OrderCargoIfc.SERVICE_PICKUP))
                {   // retrieve all order line items, not only only pickup/cancel
                    PickupOrderCargo pcar = (PickupOrderCargo) bus.getCargo();
                    allItems = pcar.getSavedOrder().getLineItems();
                    itemsLength = allItems.length;
                }
                else  // use current line items on order
                {
                   itemsLength = lineItems.length;
                   allItems = lineItems;
                }

                // mix pickup/cancel with remaining order items
                SaleReturnLineItemIfc item = null;
                StringBuffer messageDetailsStr = new StringBuffer(200);
                for (int i = 0; i < itemsLength; i++)
                {
                    item = (SaleReturnLineItemIfc)allItems[i];
                    if (lineItems.length < itemsLength)
                    {
                       for (int j = 0; j < lineItems.length; j++)
                       {
                           SaleReturnLineItemIfc currentItem = (SaleReturnLineItemIfc)lineItems[j];
                            if (item.getOrderLineReference() == currentItem.getOrderLineReference())
                            {
                                item = currentItem;
                                j = lineItems.length;
                            }
        ...
        else  // customer does NOT have email address, log warning
        {
            logger.warn( "Customer: " + cargo.getOrder().getCustomer().getFirstLastName() + " does NOT have an email address do not send email");
        }
