 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/modifytransaction/ModifyTransactionCargo.java /rgbustores_13.4x_generic_branch/2 2011/09/02 13:05:37 cgreene Exp $
 * ===========================================================================
 * NOTES
...
{                                   // begin class ModifyTransactionCargo
    ...
    **/
    protected static Logger logger = Logger.getLogger(oracle.retail.stores.pos.services.modifytransaction.ModifyTransactionCargo.class);
    ...
    **/
    ...
    //---------------------------------------------------------------------
    /**
       Constructs ModifyTransactionCargo object. <P>
    **/
    //---------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------
    /**
       Updates transaction, items with new gift registry values,
       implementing update policy. <P>
       <B>Pre-Condition</B>
       <UL>
       ...
       @param newSalesAssociate new gift registry
       @param updateAllItemsFlag flag indicating update policy
       @return int  number of items updated
    **/
    //---------------------------------------------------------------------
    public int updateTransactionSalesAssociate(EmployeeIfc newSalesAssociate,
                                               boolean updateAllItemsFlag)
    {
        // get items from transaction
        Vector lineItems = transaction.getLineItemsVector();
        ...
        {                               // begin loop through line items
            ...
            // check here to see if the element is an instance of OrderLineItemIfc
            ...
            if (!(srli instanceof OrderLineItemIfc))
            {
                ...
                if (updateAllItemsFlag)
                {
                    ...
                    srli.setSalesAssociateModifiedFlag(!newSalesAssociate.getEmployeeID().equals(transaction.getCashier().getEmployeeID()));

                    // we are updating the line items to have the same associate. as the transaction. so this should always be false.
                ...
                {
                	//update the unchanging line items to reflect whether or not they have the same sales associate as the transaction.
                    srli.setSalesAssociateModifiedAtLineItem(!newSalesAssociate.getEmployeeID().equals(srli.getSalesAssociate().getEmployeeID()));
                }
            } // end if not instance of OrderLineItemIfc
...
//
        ...
        String message =I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_SALESASSOC_LABEL, null);
        message+=Util.EOL;

        Object dataArgs[]={oldAssociateID};
        String oldRemovedSalesAssoc = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_SALESASSOC_REMOVED, dataArgs);

        Object salesAssocDataArgs[]={newSalesAssociate.getEmployeeID()};
        String salesAssocID = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_SALESASSOC_ID, salesAssocDataArgs);

        message += oldRemovedSalesAssoc;
        message += Util.EOL+salesAssocID;


        mgr.journal(transaction.getCashier().getLoginID(),
    ...
    **/
    //---------------------------------------------------------------------
    public int updateTransactionGiftRegistry(RegistryIDIfc newGiftRegistry,
                                             boolean updateAllItemsFlag)
    {
...
//                   + trans.getDefaultRegistry().getID();
                ...
                message =I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_GIFT_REG, null);
                message +=Util.EOL;

                Object dataArgs[]={trans.getDefaultRegistry().getID()};
                String giftRegisterRemoved = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_GIFTREG_REMOVED, dataArgs);
            ...
            // get items from transaction
            Vector lineItems = trans.getLineItemsVector();
            // line item object, update counter
            SaleReturnLineItemIfc srli;
            updateCounter = 0;
            // set up enumeration
...
//               + newGiftRegistry.getID();
           ...
           message =I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_GIFT_REG, null);
           message += Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.GIFT_REG, null);
     ...
     */
    public void updateTransactionTax(TransactionTaxIfc newTax,
                                     boolean updateAllItemsFlag)
    {
        ...
        if (mgr != null)
        {
            ...
            String reason = Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.REASON_CODE_LABEL, reasonCodeDataArgs);
            ...
            switch (oldTax.getTaxMode())
            {
                case TaxIfc.TAX_MODE_EXEMPT:
                {
                   ...
                   if (saleReturnTransaction.getCustomer() != null)
                   {
                       ...
                       customer = Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.CUSTOMER_ID, customerDataArgs);
                   }

                   Object taxDataArgs[] = {oldTax.getTaxExemptCertificateID()};
                   String removedTaxCert = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_REMOVED_TAX_CERT, taxDataArgs);

                   message = new StringBuilder(I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_EXEMPT, null))
                   			+customer+Util.EOL
                   			+removedTaxCert
                   			+reason+Util.EOL;
                   break;
                ...
                case TaxIfc.TAX_MODE_OVERRIDE_AMOUNT:
                {
                   ...
                   String removedTaxOverrideAmt = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_REMOVED_TAX_OVERRIDE_AMT, taxDataArgs);

                   message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_OVERRIDE, null)
                   			+Util.EOL
                ...
                case TaxIfc.TAX_MODE_OVERRIDE_RATE:
                {
                    ...
                    String removedTaxOverridePer = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_REMOVED_TAX_OVERRIDE_PER, taxDataArgs);

                    message += Util.EOL+removedTaxOverridePer
                    		+ reason+Util.EOL;
                ...
                default:
                {
                	message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_LABEL, null);

                    Object taxDataArgs[] = {formatTaxRate(oldTax.getDefaultRate())};
                    String removedTaxPer = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_REMOVED_TAX_PER, taxDataArgs);

                    message += Util.EOL
                    	+removedTaxPer+Util.EOL;

                   break;
                }
            }

            mgr.journal(saleReturnTransaction.getCashier().getEmployeeID(),
                        saleReturnTransaction.getTransactionID(),
                        message);
            ...
            switch (newTax.getTaxMode())
            {
                case TaxIfc.TAX_MODE_EXEMPT:
                {
                    ...
                    else
                    {
                        ...
                        reason = Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.REASON_CODE_LABEL, reasonCodeValueDataArgs);
                    }
                    String customer = "";
                    if (saleReturnTransaction.getCustomer() != null)
                    {
                ...
                if (newTax.getTaxExemptCertificate().getMaskedNumber() != null
                        && newTax.getTaxExemptCertificate().getMaskedNumber().length() > 0)
                    {
                        ...
                        certificate = Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_CERTIFICATE_LABEL, taxDataArgs);
                    }

                    message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_EXEMPT, null)
                                    + customer
                                    + certificate
                                    + reason
                                    + Util.EOL;
                ...
                case TaxIfc.TAX_MODE_OVERRIDE_AMOUNT:
                {
                    ...
                    message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_OVERRIDE, null)
                    +Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_OVERRIDE_AMT, null)
                    +taxOverrideAmount
                    +reason+Util.EOL;
                ...
                case TaxIfc.TAX_MODE_OVERRIDE_RATE:
                {
                    ...
                    message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_OVERRIDE, null)
                    +Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_OVERRIDE_PERC, null)
                    + " " + formatTaxRate(newTax.getOverrideRate())
                    +reason+Util.EOL;
                ...
                default:
                {
                    ...
                    String taxRestored = Util.EOL+I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_RESTORED, taxRestoredDataArgs);

                    message = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANSACTION_TAX_LABEL, null)
                    +Util.EOL+taxRestored+Util.EOL;
        ...
        {                               // begin evaluate new tax mode
            ...
            if (logger.isInfoEnabled()) logger.info(
                        "Overriding transaction tax amount ...");
            // override tax amount on transaction
            saleReturnTransaction.overrideTaxAmount(newTax.getOverrideAmount(),
                                                    updateAllItemsFlag,
                                                    newTax.getReason());
            break;
            ...
            case TaxIfc.TAX_MODE_OVERRIDE_RATE:
            if (logger.isInfoEnabled()) logger.info(
                        "Overriding transaction tax rate ...");
            // override tax rate on transaction
            saleReturnTransaction.overrideTaxRate(newTax.getOverrideRate(),
                                                  updateAllItemsFlag,
                                                  newTax.getReason());
            break;
            ...
            case TaxIfc.TAX_MODE_EXEMPT:
            if (logger.isInfoEnabled()) logger.info(
                        "Setting transaction tax exempt ...");

            // set tax exempt transaction
            saleReturnTransaction.setTaxExempt(newTax.getTaxExemptCertificateID(),
                                               newTax.getReason());
            break;
            ...
            default:
            if (logger.isInfoEnabled()) logger.info(
                        "Resetting standard tax ...");
            // check current tax mode
            if (saleReturnTransaction.getTransactionTax().getTaxMode()
                == TaxIfc.TAX_MODE_EXEMPT)
            {
    ...
    //---------------------------------------------------------------------
    /**
       Formats tax rate (corrects doubles problem). <P>
    ...
    **/
    ...
    //---------------------------------------------------------------------
    /**
       Sets UpdateParentCargoFlag flag. <P>
    ...
    **/
    ...
    //---------------------------------------------------------------------
    /**
       Returns updateParentCargoFlag flag. <P>
    ...
    **/
    ...
    //----------------------------------------------------------------------
    /**
       Already set sales associate using transaction options
       return boolean true is sales associate is already set
    **/
    //----------------------------------------------------------------------
    ...
    //---------------------------------------------------------------------
    /**
     * Retrieves list of line items <P>
    ...
    **/
    ...
    {
        // result string
        String strResult = new String("Class:  ModifyTransactionCargo (Revision " +
                                      getRevisionNumber() +
                                      ") @" +
                                      hashCode());
        return(strResult);
    ...
    **/
    ...
    /**
     * Getter for BasketDTO
     * Gets the Item Basket DTO which conaints item
    ...
    /**
     * Sets the Basket DTO into the Cargo
     * Setter for BasketDTO
     */
...
}                                       // end class ModifyTransactionCargo
