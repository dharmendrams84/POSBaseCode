...
public class ItemBasketSite extends PosSiteActionAdapter
{
  ...
  public void arrive(BusIfc bus)
  {
    ...
    if(basket.getSeqNumber() != -1)
    {
      String itemId = basket.getsaleReturnSpecifiedItem().getItemID();

        {
          journalBasketItems(basket.getsaleReturnSpecifiedItem(),basket.getTransaction(),cargo);
        }
    }
    String returnedLetter = bus.getCurrentLetter().getName() ;
    boolean isItemRemoved = false;

    if(returnedLetter != null && (CommonLetterIfc.UNDO.equals(returnedLetter) ||  CommonLetterIfc.INVALID.equals(returnedLetter) || CommonLetterIfc.CANCEL.equals(returnedLetter)))
    {

     // pluItems = cargo.getPLUItems();
      SaleReturnLineItemIfc saleReturnItem = basket.getsaleReturnSpecifiedItem();
      String currentItemIdTraversed = saleReturnItem.getItemID();
      if(currentItemIdTraversed != null)
      {
        HashMap pluItemMap = basket.getPLUItemMap();
        pluItemMap.remove(basket.getsaleReturnSpecifiedItem().getItemID());
        ArrayList lineItems = basket.getSaleReturnItems();
        lineItems.remove(basket.getSeqNumber());
        sizeOfBasket = basket.getTotalItemsInBasket();

        Vector lineItemsVector = cargo.getTransaction().getLineItemsVector();
        lineItemsVector.remove(basket.getSeqNumber());
        isItemRemoved = true;

      }
      //pluItems.remove(cargo.getPLUItemID());
      //itemIDList.remove(cargo.getPLUItemID());
     // unknownItemsFound.remove(cargo.getPLUItemID());
    }
    if (basket != null)
    {
      if(basket.getSeqNumber() <= sizeOfBasket && !isItemRemoved)
      {

          basket.incrementSeqNumber();
      }

      if (basket.getSeqNumber() >= basket.getTotalItemsInBasket())
      {
        ...
        entry.append(I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.TRANS_LABEL, dataArgs,
            journalLocale));
        ...
        entry.append(Util.EOL);
        journal.journal(cargo.getRegister().getWorkstation().getStoreID(), cargo.getRegister().getWorkstation().getWorkstationID(), cargo.getOperator()
            .getEmployeeID(), null, null, entry.toString());
  ...
  public void journalBasketItems(SaleReturnLineItemIfc item,SaleReturnTransactionIfc transaction,ModifyTransactionCargo cargo)
  {
