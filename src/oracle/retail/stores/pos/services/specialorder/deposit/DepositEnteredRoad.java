 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/specialorder/deposit/DepositEnteredRoad.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:03 mszekely Exp $
 * ===========================================================================
 * NOTES
...
{ // begin class DepositEnteredRoad
    ...
    { // begin traverse()
        ...
        CurrencyIfc total = orderStatus.getTotal();
        orderStatus.setBalanceDue(total.subtract(deposit));
        ...
        // journal entry for pickup/delivery status of entry..Nilesh.
        ...
        if (orderTransaction.getOrderType() == OrderConstantsIfc.ORDER_TYPE_ON_HAND)
        {
            ...
            String pickupTag = I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.PICKUP_TAG,
                    dataObject);
            ...
            for (int i = 0; i < items.length; i++)
            {
                ...
                if (lineItem.getOrderItemStatus().getItemDispositionCode() == OrderLineItemIfc.ORDER_ITEM_DISPOSITION_PICKUP)
                {
                    ...
                    sb.append(Util.EOL).append(itemLabel).append(((SaleReturnLineItemIfc)items[i]).getItemID().trim())
                            .append(Util.EOL).append(pickupTag.concat(" " + pickupDate)).append(Util.EOL);
                }
                else if (lineItem.getOrderItemStatus().getItemDispositionCode() == OrderLineItemIfc.ORDER_ITEM_DISPOSITION_DELIVERY)
                {
                    String deliveryDate = lineItem.getOrderItemStatus().getDeliveryDate().toFormattedString();
                    sb.append(Util.EOL).append(itemLabel).append(((SaleReturnLineItemIfc)items[i]).getItemID().trim())
