 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/returns/returncommon/ReturnUtilities.java /rgbustores_13.4x_generic_branch/4 2011/08/11 19:22:51 ohorne Exp $
 * ===========================================================================
 * NOTES
...
public class ReturnUtilities
{
        ...
        SaleReturnTransactionIfc saleTransaction)
    {
        if (item.getOrderItemStatus() != null)
        {
            if (item.getOrderItemStatus().getStatus().getStatus()
                == OrderLineItemIfc.ORDER_ITEM_STATUS_CANCELED)
            {
    ...
    public static void setTransactionDiscounts(SaleReturnLineItemIfc srli, BigDecimal quantity)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            ritda.initialize(discountAmount, DiscountRuleConstantsIfc.DISCOUNT_METHOD_AMOUNT, itda.getReason());
            ritda.setAssignmentBasis(itda.getAssignmentBasis());
            ritda.setOriginalDiscountMethod(itda.getDiscountMethod());
