 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/reprintreceipt/GiftReceiptCheckEligibilityAisle.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:30 mszekely Exp $
 * ===========================================================================
 * NOTES
...
//--------------------------------------------------------------------------
/**
    This class retrieves the selection from the  UIModel and
    saves the selection list in the cargo. Checks to see if 
    gift receipt is being applied to a damage discounted item.
...
{    
    ...
    //--------------------------------------------------------------------------
    /**
        This method retrieves the selected rows from the UIModel and
        saves the selection list in the cargo. Each item selected is
        checked to see if gift receipt is being applied to a damage
        discounted item. If so, an error dialog is displayed. The 
        GiftReceiptEligible letter is mailed.
    ...
    **/
    ...
    public void traverse(BusIfc bus)
    {
        ...
        for (int i=0; i < index.length; i++)
        {
            ...
            if(lineItems[selectedIndex].hasDamageDiscount()||numberReturnable.compareTo(new BigDecimal("0.00")) <= 0)
            {
                itemCount++;
                if(filledSlots < 3)
                {
                    args[filledSlots] = lineItems[selectedIndex].getItemDescription(LocaleMap.getLocale(LocaleConstantsIfc.USER_INTERFACE));
                    filledSlots++;
