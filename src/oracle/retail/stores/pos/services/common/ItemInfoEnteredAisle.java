 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/common/ItemInfoEnteredAisle.java /rgbustores_13.4x_generic_branch/5 2011/06/15 23:00:02 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class ItemInfoEnteredAisle extends LaneActionAdapter
{
    ...
    public void traverse(BusIfc bus)
    {
        ...
        {
            String formattedSizeNumber =
              PLUItemUtility.getFormattedSizeNumber(model.getItemSize());
            ((ItemSizeCargoIfc)cargo).setItemSizeCode(formattedSizeNumber);
        }
        ...
        if (Util.isEmpty(model.getItemSerial()) ||
            Util.isObjectEqual(model.getQuantity(), Util.I_BIG_DECIMAL_ONE) )
        {
            ...
            /* Only want to show the "Item Not Found" item on the customer
             * interface device when the item is being added to the transaction,
             * which is what happens in PosCargo. We also come through here for
             * returns, but cargo is of type AbstractReturnItemCargo which
             * doesn't add the item to the transaction. For returns, items are
             * added to the transaction in CreateAndUpdateTransactionSite. Donot
             * print the item on the device for an external order item since it
             * will be printed later in PrintItemRoad.
             */
            if (!cargo.isExternalOrder())
            {
                if (cargo instanceof SaleCargoIfc)
                {
                    SaleCargoIfc scargo = (SaleCargoIfc)bus.getCargo();
                    WorkstationIfc workstation = scargo.getRegister().getWorkstation();
                    CPOIPaymentUtility.getInstance().addItem(workstation, item, ((SaleCargoIfc)cargo).getTransaction());
                }
