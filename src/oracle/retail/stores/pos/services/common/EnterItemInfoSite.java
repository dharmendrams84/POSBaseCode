 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/common/EnterItemInfoSite.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:05:52 mszekely Exp $
 * ===========================================================================
 * NOTES
...
//--------------------------------------------------------------------------
/**
    This site displays the ITEM_NOT_FOUND screen to allow
    the user to enter the required information for the item.
    <p>
...
public class EnterItemInfoSite extends PosSiteActionAdapter
{
    ...
    **/
    ...
    //----------------------------------------------------------------------
    /**
        Displays the ITEM_NOT_FOUND form to allow the user
    ...
    **/
    ...
    //----------------------------------------------------------------------
    /**
        Initializes the bean model<P>
        @param  bus Service Bus
        @return ItemNotFoundBeanModel item not found bean model with all
    ...
    **/
    ...
    public ItemNotFoundBeanModel getModelWithAllSettings(BusIfc bus)
    {
        ...
        if (cargo.getPLUItem() instanceof UnknownItem
            && bus.getCurrentLetter() != null && bus.getCurrentLetter().getName().equalsIgnoreCase("Ok"))
        {
    ...
    /**
     * Assuming the PLU item is an instance of an UnknownItem, will set the model with as many settings
     * as possible from the cargo.
     */
    // SCR 4895
    private static void setModelWithUnknownItemSettings(ItemNotFoundBeanModel model, PLUCargoIfc cargo)
    {
        ...
        // This seems to have no effect.  And on Returns of an UnknowItem, it blows up.
    ...
    **/
    ...
    public ItemNotFoundBeanModel getModel(PLUCargoIfc cargo)
    {
        ...
        Locale locale = LocaleMap.getLocale(LocaleConstantsIfc.USER_INTERFACE);
        String unit = utility.retrieveText(POSUIManagerIfc.ITEM_NOT_FOUND_SPEC,
                                           BundleConstantsIfc.COMMON_BUNDLE_NAME,
                                           UNIT_OF_MEASURE_TAG,
                                           UNIT_OF_MEASURE_TEXT,
                                           locale);
        model.setDefaultUOM(unit);
        //these default values will be displayed for unitofmeasure, quantity, and price
    ...
    **/
    ...
    protected String getReason(PLUCargoIfc cargo)
    {
        ...
        {
            reason=utility.retrieveText(POSUIManagerIfc.STATUS_SPEC,
                                        BundleConstantsIfc.COMMON_BUNDLE_NAME,
                                        ITEM_NOT_FOUND_SCREEN_NAME_TAG,
                                        ITEM_NOT_FOUND_SCREEN_NAME_TEXT);
        }
        else
        {
