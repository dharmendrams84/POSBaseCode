 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/modifyitem/LookupServiceItemsSite.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:25 mszekely Exp $
 * ===========================================================================
 * NOTES
    ...
    **/
    ...
    public void arrive(BusIfc bus)
    {
        ...
        try
        {
            ...
            pluRequestor.addRequestType(PLURequestor.RequestType.Price);
            pluRequestor.addRequestType(PLURequestor.RequestType.LocalizedDescription);
            ...
            serviceItems = pluTransaction.getServiceItems(inquiry);

            // eliminate duplicates and sorts based on Locale

            Collator collate = Collator.getInstance(locale);
            TreeMap items = new TreeMap(collate);
            for (int i = 0; i < serviceItems.length; i++)
            {
                if (items != null && !items.containsKey(serviceItems[i].getDescription(locale)))
                {
                  items.put(serviceItems[i].getItemID(), serviceItems[i]);
                }
            }
            serviceItems = new PLUItemIfc[items.size()];
            Iterator ki = items.keySet().iterator();
            int j = 0;
            while (ki.hasNext())
            {
                String key = (String) ki.next();
                serviceItems[j++] = (PLUItemIfc) items.get(key);
            }

            cargo.setServiceItemList(serviceItems);
