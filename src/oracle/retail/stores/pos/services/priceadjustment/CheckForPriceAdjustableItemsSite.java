 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/priceadjustment/CheckForPriceAdjustableItemsSite.java /rgbustores_13.4x_generic_branch/3 2011/10/25 10:22:47 rsnayak Exp $
 * ===========================================================================
 * NOTES
...
public class CheckForPriceAdjustableItemsSite extends PosSiteActionAdapter
{
    ...
    public void arrive(BusIfc bus)
    {
        ...
        try
        {
            ...
            CustomerIfc customer = originalTransaction.getCustomer();
            ...
            throws DataException
    {
        ...
        for (int i = 0; i < lineItems.size(); i++)
        {
            ...
            PLUItemIfc currentItem = pluTransaction.getPLUItem(searchCriteria);
            ...
            if(originalTransaction instanceof OrderTransactionIfc)
            {
                        ...
                        && transactionType == TransactionIfc.TYPE_ORDER_INITIATE)
                {
                    if (item.getOrderItemStatus().getItemDispositionCode() == OrderLineItemIfc.ORDER_ITEM_DISPOSITION_DELIVERY
                            || item.getOrderItemStatus().getItemDispositionCode() == OrderLineItemIfc.ORDER_ITEM_DISPOSITION_PICKUP)
                    {
    ...
    /**
     * Copies tax info over to the current line item
     *
     * @param originalLineItem Original line item
     * @param currentLineItem Current line item
     * @deprecated in version 13.3; the item should be taxed at the rate for the
     * current store, not the original line item's, which could be from another
     * store in a different set of tax authorities.
     */
    public void copyTaxOver(SaleReturnLineItemIfc originalLineItem, SaleReturnLineItemIfc currentLineItem)
    {
        NewTaxRuleIfc[] originalTaxRules = originalLineItem.getPLUItem().getTaxRules();
        if (originalTaxRules != null)
        {
            NewTaxRuleIfc[] currentTaxRules = new NewTaxRuleIfc[originalTaxRules.length];
            for (int i = 0; i < currentTaxRules.length; i++)
            {
                currentTaxRules[i] = (NewTaxRuleIfc)originalTaxRules[i].clone();
            }
            currentLineItem.getPLUItem().setTaxRules(currentTaxRules);
        }
    }
