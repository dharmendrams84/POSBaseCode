 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/tender/tdo/PaidUpTDO.java /rgbustores_13.4x_generic_branch/3 2011/08/16 12:23:14 rsnayak Exp $
 * ===========================================================================
 * NOTES
                       ...
                       implements TDOUIIfc
{
    ...
    public POSBaseBeanModel buildBeanModel(HashMap attributeMap)
    {
        ...
        if (txnRDO.getTenderTotalAmountPlusChangeDue()!= null)
        {
          ...
          model.getTotalsModel().setTendered(txnRDO.getTenderTotalAmountPlusChangeDue().toFormattedString());
            ...
            {
                model.getTotalsModel().setBalanceDue( (txnRDO.getTenderTransactionTotals().getGrandTotal().subtract(txnRDO.getTenderTotalAmountPlusChangeDue())).toFormattedString());
            }
        }
        String arg = change;
    ...
    public String formatPoleDisplayLine2(RetailTransactionADOIfc txnADO)
    {
        ...
        // change message if the transaction is a desposit
        ...
        {
            balance = txnRDO.getTenderTransactionTotals().getBalanceDue();
            changeDue = utility.retrieveLineDisplayText("BalanceDueText", "BALANCE DUE");
        }
        else
        {
            balance = txnRDO.calculateChangeDue();
            changeDue = utility.retrieveLineDisplayText("ChangeDueText", "CHANGE DUE");
        }
        txnRDO.getTenderTransactionTotals().setChangeDue(balance);
