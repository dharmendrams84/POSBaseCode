 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/tender/capturecustomerinfo/CaptureCustomerInfoSite.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:05:46 mszekely Exp $
 * ===========================================================================
 * NOTES
...
/**
 * @author kph Used for the capture customer info use-case. Displays a screen
 *         that will ask for customer information. Capture Customer Info use
 *         case. Written by kph. This use case capture one-time customer
 *         information for a transaction. This use case should only be executed
 *         when the "360Customer" parameter is set to "N". **NOTE** : The
 *         shuttle coming to this service should load the
 *         CaptureCustomerInfoCargo object with 2 things: 1. the transaction
 *         object. 2. the tender type.
 */
public class CaptureCustomerInfoSite extends PosSiteActionAdapter
{
    ...
    public void arrive(BusIfc bus)
    {
        ...
        // String onAmtDueOnly = util.getParameterValue("NegativeAmtDue", "N");
        custModel = (CustomerInfoBeanModel)CustomerUtilities.getCustomerInfo(customer, utility, pm);
        ...
        else
        {
            ...
            ((CaptureCustomerInfoTDO)tdo).customerToModel(model, bus, (CaptureCustomerIfc)cargo.getCustomer());
        }
        if (custModel != null && custModel.getPhoneList() != null)
        {
        ...
        if (cargo.getTenderType() == TransactionConstantsIfc.TYPE_SEND)
        {
            if (cargo.getTransaction() instanceof SaleReturnTransaction)
            {
                ...
                {
                    if (saleReturnTransaction.getCustomer().isBusinessCustomer())
                    {
            ...
            CustomerIfc customer, UtilityManagerIfc utility, ParameterManagerIfc pm)
    {
        ...
        if (customer.isBusinessCustomer())
        {
            ...
            model.setOrgName(companyName);
            model.setFirstName(companyName);
            model.setBusinessCustomer(true);
        }
        else
        {
        ...
        if (!addressVector.isEmpty())
        {
            ...
            if (address != null)
            {
                ...
                model.setExtPostalCode(address.getPostalCodeExtension());
            }
        }
        else
        {
