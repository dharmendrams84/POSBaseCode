 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/dailyoperations/common/LookupRegisterSite.java /main/14 2011/02/16 09:13:26 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class LookupRegisterSite extends PosSiteActionAdapter
{
    ...
    public void arrive(BusIfc bus)
    {
        ...
        { // begin evaluate workstation IDs and read
            ...
            if (register != null)
            {
                ...
                // These are the tills we retrieved from Hard Totals. Below we are going to retrieve
                // the Tills for this register from the database (minus financial data) and overlay what
                // we found onto the Hard Totals' Tills. This composite will be stored in the register
                // object in the cargo
                hardTotalsTills = register.getTills();
            }
    ...
    /**
     * Get the last training transaction sequence number. compare the two from db and the
     * existing one from register. The one from register may be the previous transaction sequence
     * number or the number get from hardtotals.
     * @param register    register object
     * @return             the sequence number
     * @deprecated as of 13.3 use non-training mode sequence instead
     */
    protected int getLastTrainingTransactionSequenceNumber(RegisterIfc register)
    {
        int databaseTransactionSequenceNumber = 0;
        try
        {
            TransactionReadDataTransaction dt = null;

            dt = (TransactionReadDataTransaction) DataTransactionFactory.create(DataTransactionKeys.TRANSACTION_READ_DATA_TRANSACTION);

            databaseTransactionSequenceNumber = dt.getLastTrainingTransactionSequenceNumber(register);
        ...
        return getLastTransactionSequenceNumberForTraningMode(register, databaseTransactionSequenceNumber);
    }

    /**
     * Compare the two sequence number and return the bigger one.
     * @param register                              register object
     * @param databaseTransactionSequenceNumber     the last traing mode transaction sequence number from data base
     * @return                                      the sequence number
     * @deprecated as of 13.3 use non-training mode sequence instead
     */
    protected int getLastTransactionSequenceNumberForTraningMode(RegisterIfc register, int databaseTransactionSequenceNumber)
    {
        int seqNo = register.getTrainingTransactionSequenceNumber();

        return databaseTransactionSequenceNumber > seqNo ? databaseTransactionSequenceNumber : seqNo;
    }

    /**
     * Get the last training customer sequence number. compare the two from db and the
     * existing one from register. The one from register may be the previous customer sequence
     * number or the number get from hardtotals.
     * @param register    register object
     * @return             the customer sequence number
     * @deprecated as of 13.3 use non-training mode sequence instead
     */
    protected int getLastTrainingCustomerSequenceNumber(RegisterIfc register)
    {
        int databaseCustomerSequenceNumber = 0;
        try
        {
            FinancialTotalsDataTransaction db = null;

            db = (FinancialTotalsDataTransaction)DataTransactionFactory
                    .create(DataTransactionKeys.FINANCIAL_TOTALS_DATA_TRANSACTION);

            databaseCustomerSequenceNumber = db.getLastTrainingCustomerSequenceNumber(register);
        ...
        return getLastCustomerSequenceNumberForTraningMode(register, databaseCustomerSequenceNumber);
    }

    /**
     * Compare the two sequence number and return the bigger one.
     * @param register                              register object
     * @param databaseCustomerSequenceNumber     the last traing mode customer sequence number from data base
     * @return                                      the sequence number
     * @deprecated as of 13.3 use non-training mode sequence instead
     */
    protected int getLastCustomerSequenceNumberForTraningMode(RegisterIfc register, int databaseCustomerSequenceNumber)
    {
        int seqNo = register.getTrainingCustomerSequenceNumber();

        return databaseCustomerSequenceNumber > seqNo ? databaseCustomerSequenceNumber : seqNo;
    }
}
