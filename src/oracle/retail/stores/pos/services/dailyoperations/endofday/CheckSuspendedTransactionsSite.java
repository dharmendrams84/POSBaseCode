 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/dailyoperations/endofday/CheckSuspendedTransactionsSite.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:22 mszekely Exp $
 * ===========================================================================
 * NOTES
...
{                                                                               // begin class CheckSuspendedTransactionsSite
    ...
    **/
    ...
    {                                                                   // begin arrive()
        ...
        /*if no genuine transactions are suspended, then the store will close anyway,
        we should go ahead and automatically cancel all the training mode suspended trans
        that may exist. Training mode suspended trans should not hinder the operator in
        closing of the store */
        if(!genuineTransExist)
        {
            try
            {
                ...
                writeTransaction.cancelSuspendedTransactions
                (cargo.getStoreStatus().getStore().getStoreID(),
                 cargo.getStoreStatus().getBusinessDate());
            }
            catch (DataException e)
            {
                // if exception is not no-data, it's an error
