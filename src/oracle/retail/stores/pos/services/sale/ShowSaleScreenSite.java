 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/sale/ShowSaleScreenSite.java /rgbustores_13.4x_generic_branch/12 2011/09/30 12:44:22 mchellap Exp $
 * ===========================================================================
 * NOTES
...
public class ShowSaleScreenSite extends PosSiteActionAdapter
{
    ...
    public void arrive(BusIfc bus)
    {
        ...
        {
            cargo.getTransaction().setTransactionStatus(TransactionConstantsIfc.STATUS_IN_PROGRESS);
        }
        ...
        localModel.setButtonEnabled(CommonActionsIfc.CUSTOMER, cargo.isCustomerEnabled());

        localModel.setButtonEnabled(CommonActionsIfc.SCANSHEET, Gateway.getBooleanProperty(APPLICATION_PROPERTY_GROUP_NAME, "enableScanSheet", false));
        ...
        if (transaction == null)
        {
            ...
            try
            {
                String houseAccountAccepted = pm.getStringValue("HouseCardsAccepted");
                boolean enableHouseAccount = "Y".equalsIgnoreCase(houseAccountAccepted);
                localModel.setButtonEnabled(CommonActionsIfc.HOUSEACCOUNT, enableHouseAccount);
            }
        ...
        else
        {
            ...
            if (totals.isTransactionLevelSendAssigned())
            {
                ShippingMethodIfc sendShippingMethod = totals.getSendPackages()[0].getShippingMethod();
        ...
        boolean transactionReentryModeOn = cargo.getRegister().getWorkstation().isTransReentryMode();
     ...
     * @param utility
     */
    protected void setLineDisplay(BusIfc bus, SaleCargoIfc cargo, POSDeviceActions pda, SaleReturnTransactionIfc transaction, UtilityManagerIfc utility)
    {
        try
        {
            if (transaction != null)
            {
                // update running tax and total on line display since
                // modifications
     ...
     * @param itemList
     */
    protected void setCPOIDisplay(BusIfc bus, SaleCargoIfc cargo, POSDeviceActions pda,
            SaleReturnTransactionIfc transaction, ArrayList<AbstractTransactionLineItemIfc> itemList)
    {
        try
        {
            ...
            {
                CPOIPaymentUtility cpoiPaymentUtility = CPOIPaymentUtility.getInstance();
                if(!cpoiPaymentUtility.isScrollingReceiptInSession())
                {
                    cpoiPaymentUtility.beginScrollingReceipt(workstation, true);
                }
                if (cargo.isRefreshNeeded())
                {
                    cpoiPaymentUtility.refreshItems(workstation, itemList, transaction);
                    cargo.setRefreshNeeded(false);
                }

                // during a transaction we might have a linked customer, so use their locale.
                if(transaction.getCustomer() != null && transaction.getCustomer().getPreferredLocale() != null)
                {
                    locale = transaction.getCustomer().getPreferredLocale();
                }

                cpoiPaymentUtility.updateTotals(cargo.getRegister().getWorkstation(),
                                                locale,
                                                transaction.getTransactionTotals().getQuantityTotal().intValue(),
                                                transaction.getTransactionTotals().getDiscountTotal(),
                                                transaction.getTransactionTotals().getTaxTotalUI(),
                                                transaction.getTransactionTotals().getBalanceDue());
     ...
     */
    protected boolean islineItemContainReturnableItem(AbstractTransactionLineItemIfc items[])
    {
        int noOfReturnLineItem = 0;
        boolean result = false;
        for (int i = 0; i < items.length; i++)
        {
            SaleReturnLineItemIfc saleReturnItem = (SaleReturnLineItemIfc)items[i];
            if (saleReturnItem.isReturnLineItem())
            {
                noOfReturnLineItem++;
            }
        }
        if (noOfReturnLineItem >= 1)
        {
            result = true;
        }
     ...
     * @return boolean result
     */
    protected boolean islineItemContainSendItem(AbstractTransactionLineItemIfc items[])
    {
        int noOfReturnLineItem = 0;
        boolean result = false;
        for (int i = 0; i < items.length; i++)
