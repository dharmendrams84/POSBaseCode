 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/services/sale/SaleCargo.java /rgbustores_13.4x_generic_branch/6 2012/02/27 18:16:37 asinton Exp $
 * ===========================================================================
 * NOTES
        ...
        SaleCargoIfc, ItemSizeCargoIfc, TimedCargoIfc, PLUCargoIfc
{
    ...
    public void completeItemNotFound()
    {
        ...
        if (journal != null)
        {
            ...
            // sb.append(Util.EOL).append("  Dept.: ")
            // .append(pluItem.getDepartmentID())
            // .append(Util.EOL).append("  Item Not Found");
            //
            //
            ...
            journal.journal(getEmployee().getLoginID(), transaction.getTransactionID(), sb.toString());
        }
    }
    ...
    {
        /*
         * The transaction is checked to see if there is a reason to not enable
         * the customer button. As of release 5, the customer button will be
         * disabled if an order item is inclueded in the transaction.
         */
        boolean enabled = true;
        if (transaction != null)
        {

            Enumeration<AbstractTransactionLineItemIfc> enumer = transaction.getLineItemsVector().elements();
            while (enumer.hasMoreElements())
            {
                if (enumer.nextElement() instanceof OrderLineItemIfc)
                {
                    enabled = false;
                }
            }
