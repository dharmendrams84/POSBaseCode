 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/receipt/VATHelper.java /main/17 2011/02/27 20:37:37 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class VATHelper
{
    ...
    public TaxInformationIfc[] getVATSummary()
    {
        if (vatSummary == null)
        {
            HashMap<String,TaxInformationIfc> taxRateCodeMap = new HashMap<String,TaxInformationIfc>();
            ...
            // accumulate the totals for each VAT code from the send package line items
            SendPackageLineItemIfc[] packages = transaction.getTransactionTotals().getSendPackages();
            for (int i = 0; i < packages.length; i++)
            {
                SendPackageLineItemIfc spli = packages[i];
                ShippingMethodIfc shippingMethod = spli.getShippingMethod();
                TaxInformationContainerIfc taxContainer = spli.getTaxInformationContainer();
                TaxInformationIfc[] taxInfomations = taxContainer.getOrderedTaxInformation();
                if(taxInfomations.length > 0 && taxInfomations[0] != null)
                {
                    TaxInformationIfc ti = taxInfomations[0];
                    if(ti.getInclusiveTaxFlag())
                    {
                        ...
                        // get amounts
                        CurrencyIfc includingTax = shippingMethod.getCalculatedShippingCharge();
                        CurrencyIfc tax = taxContainer.getInclusiveTaxAmount();
                        CurrencyIfc excludingTax = includingTax.subtract(tax);
                        // accumulate for each vat code
                        updateInclusiveTaxableAmount(taxInfo, includingTax);
    ...
    public void journalVATTaxTotals(StringBuffer sb, SaleReturnTransactionIfc saleReturnTransaction, boolean negate)
    {
        ...
        HashMap<Integer,String> taxRateCodeMap = new HashMap<Integer,String>();
        HashMap taxableAmountMap = new HashMap();
        HashMap taxAmountMap = new HashMap();
        ...
        // accumulate the totals for each VAT code from the send package line items
        SendPackageLineItemIfc[] packages = saleReturnTransaction.getTransactionTotals().getSendPackages();
        for (int i = 0; i < packages.length; i++)
        {
            SendPackageLineItemIfc spli = packages[i];
            ShippingMethodIfc shippingMethod = spli.getShippingMethod();
            ...
            if(taxInfomations.length > 0 && taxInfomations[0] != null)
            {
                ...
                if(ti.getInclusiveTaxFlag())
                {
                    ...
                    String rateName = taxRateCodeMap.get(taxGroupID);
                    if(rateName == null)
                    {
                        taxRateCodeMap.put(taxGroupID, vat + " " + vatCode);
                    }
                    // get amounts
                    CurrencyIfc incTax = shippingMethod.getCalculatedShippingCharge();
                    CurrencyIfc tax = taxContainer.getInclusiveTaxAmount();
                    CurrencyIfc exTax = incTax.subtract(tax);
                    // accumulate totals
                    taxAmountTotal = taxAmountTotal.add(tax);
    ...
    protected void printItemVATSummary(StringBuffer buffer, CurrencyIfc tax, CurrencyIfc price, CurrencyIfc excludingTax, String rateCode, String vatText)
    {
        ...
        dataArgs = new Object[]{vatText,excludingTax.toGroupFormattedString()};
        buffer.append(I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.PRICE_EXCLUDING_LABEL, dataArgs));
        buffer.append(Util.EOL);

        dataArgs = new Object[]{vatText,price.toGroupFormattedString()};
    ...
    public void journalVATKitComponentLineItemsTax(StringBuffer buffer, KitComponentLineItemIfc[] kitComponentLineItems)
    {
        ...
        {
            buffer.append(Util.EOL);
            HashMap rateCodeMap = new HashMap();
            HashMap priceIncTaxMap = new HashMap();
            HashMap priceExclTaxMap = new HashMap();
            HashMap taxTotalMap = new HashMap();
    ...
    {
        if (taxInfo.getTaxableAmount() == null)
        {
            taxInfo.setTaxableAmount(excludingTax);
        }
        else
        {
            taxInfo.setTaxableAmount(excludingTax.add(taxInfo.getTaxableAmount()));
        }
    }
    ...
    private void updateTaxAmount(TaxInformationIfc taxInfo, CurrencyIfc tax)
    {
        ...
        {
            taxInfo.setTaxAmount(tax.add(taxInfo.getTaxAmount()));
        }
    }
