 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/ui/beans/ShippingMethodBeanModel.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:57 mszekely Exp $
 * ===========================================================================
 * NOTES
...
public class ShippingMethodBeanModel extends MailBankCheckInfoBeanModel
{
    ...
    {
        if (!offline && getSelectedShipMethod() != null)
        {
            
            // calculate shiping charge base on parameter
            CurrencyIfc baseCharge = getSelectedShipMethod().getBaseShippingCharge();
            CurrencyIfc weightRate = getSelectedShipMethod().getShippingChargeRateByWeight();
            if (calculationType.compareTo(ShippingMethodConstantsIfc.FLAT_RATE) == 0)
            {
                shippingCharge = getSelectedShipMethod().getFlatRate();
            }
    ...
    public void setShippingCharge(CurrencyIfc value)
    {
        if (getSelectedShipMethod() != null)
        {
            ...
            getSelectedShipMethod().setCalculatedShippingCharge(shippingCharge);
        }
    }
