 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/ui/beans/POSBaseBeanModel.java /rgbustores_13.4x_generic_branch/4 2011/09/01 11:50:28 cgreene Exp $
 * ===========================================================================
 * NOTES
 ...
 */
public class POSBaseBeanModel implements UIModelIfc, ScreenTimeoutIfc
{
    ...
    /*
     * Should the container be unlocked by this model's update? Some updates to
     * the UI are performed asynchronously and should not unlock user input. For
     * instance the on/offline status can change at any time. Unlocking user
     * input at some points can cause POS to crash when the user's input is
     * unexpected.
     */
    protected boolean unlockContainer = true;
    ...
    {
        // initialize timer with timeout interval
        if (TimeoutSettingsUtility.isTransactionActive())
        {
            // If a transaction is open, initialize the timeer with
            // TimeoutInactiveWithTransaction parameter
            timerModel = new DefaultTimerModel(true, TimerModelIfc.TRANS_WITH);
        }
        else
        {
            // Initialize the timeer with TimeoutInactiveWithoutTransaction
            // parameter
            timerModel = new DefaultTimerModel(false, TimerModelIfc.TRANS_WITHOUT);
        }
    }
    ...
    /*
     * Should the container be unlocked by this model's update?
     * @return unlockContainer
     */
    public boolean isUnlockContainer()
    {
        return unlockContainer;
    }
    ...
    /*
     * Should the container be unlocked by this model's update?
     * @param unlockContainer
     */
    public void setUnlockContainer(boolean unlockContainer)
    {
