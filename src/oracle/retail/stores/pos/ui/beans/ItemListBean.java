 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/ui/beans/ItemListBean.java /rgbustores_13.4x_generic_branch/2 2011/08/16 14:47:00 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class ItemListBean extends ListBean
{
    ...
    protected void loadImage(final ItemImageIfc itemImage, final int row)
    {
        ...
        new SwingWorker(Integer.toString(row))
        {
            ...
            {
                Image image = null;
                String location = itemImage.getImageLocation();
                ...
                try
                {
                    ...
                    {
                        image = Toolkit.getDefaultToolkit().createImage(itemImage.getImageBlob());
                        image = image.getScaledInstance(50, 50, Image.SCALE_FAST);
                    }
                    ...
                    {
                        itemImage.setEmptyImage(true);
                    }

                    itemImage.setLoadingImage(false);
                    if (image != null)
                    {
                        itemImage.setImage(new ImageIcon(image));
                    }
                }
                catch (Exception ex)
                {
                    logger.warn("Unable to load item image for " + itemImage, ex);
                    itemImage.setImageError(true);
                }
                finally
                {
