 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/ui/beans/ReturnItemInfoBeanModel.java /rgbustores_13.4x_generic_branch/1 2011/05/05 14:06:54 mszekely Exp $
 * ===========================================================================
 * NOTES
...
//----------------------------------------------------------------------------
/**
   This class is used with the ReturnItemInfoBean class. <p>
...
public class ReturnItemInfoBeanModel extends ReasonBeanModel
{
    ...
    /** Gift card balance.   **/
    protected BigDecimal giftCardBalance = Util.I_BIG_DECIMAL_ZERO;
    ...
    /*
     * All the following data members have been deprecated. 
     */
    /**  Tax rate   */ 
    /** @deprecated in version 13.3; not used */
    protected BigDecimal taxRate = Util.I_BIG_DECIMAL_ZERO;

    /** Associated status IDs */
    /** @deprecated in version 13.3; not used */
    protected Vector associatedStateIds = new Vector();
    
    /** Associated status Names */
    /** @deprecated in version 13.3; not used */
    protected Vector associatedStateNames = new Vector();

    /** This boolean indicates that only the Quantity and Reason Code should be enabled. **/
    /** @deprecated in version 13.3; not used */
    protected boolean enableOnlyQuantityReason = false;
    ...
    /** This flag indicates this item was from a retrieved transaction */
    /** @deprecated in 13.3 */
    protected boolean isItemFromRetrievedTransaction = false;
    ...
    */
    ...
    //----------------------------------------------------------------------------
    /**
     * Returns whether this item is a unit of measure (allows fractional pieces).
     * Most things in this world are not.
    ...
    //----------------------------------------------------------------------------
    /**
        Retrieves unitOfMeasure text. <P>
    ...
    **/
    ...
    {
        // If we have a unit of measure item, flesh its quantity to 2 decimal places.
        if (isUOM())
        {
            int scale = quantity.scale();  // how many decimal digits there are. 
            if (scale == 0 || scale == 1)
            {
                quantity = quantity.multiply(Util.I_BIG_DECIMAL_ONE);  // "1.00"
            }    
        }
    ...
    **/
    ...
    //----------------------------------------------------------------------------
    /**
        Retrieves EnableOnlyQuantityReason. <P>
        @return boolean
    **/
    //----------------------------------------------------------------------------
    ...
    {                                  
        return quantityEnabled 
            && !priceEnabled 
            && !storeNumberEnabled
            && !salesAssociateEnabled
            && !receiptNumberEnabled
            && !unitOfMeasureEnabled
            && !itemSizeEnabled
            && !serialNumberEnabled;
    }                                   
    ...
    //----------------------------------------------------------------------------
    /**
        Sets EnableOnlyQuantityReason. <P>
        @param value Whether this property is <code>true</code> or <code>false</code>.
    **/
    //----------------------------------------------------------------------------
...
// Gift Card related methods
    ...
    */
    ...
    //  ------------------------------------------------------------------------
     ...
     //---------------------------------------------------------------------
     /**
         Retrieves the restocking fee. <P>
    ...
    **/
    ...
    /* 
     * Modifications for the External Order Item integration required some
     * changes to this class and prompted clean up of unused methods and
     * data members.  All deprecated methods have been moved to the end of
     * the class. 
     */

    //---------------------------------------------------------------------
    /**
        Retrieves the associated state Ids. <P>
        @return associatedStateIds as HashMap
        @deprecated in version 13.3; not used 
    */
    //---------------------------------------------------------------------
    public Vector getAssociatedStateIds()
    {
        return(associatedStateIds);
    }

    //---------------------------------------------------------------------
    /**
        Sets the associated state Ids.  <P>
        @param value as HashMap 
        @deprecated in version 13.3; not used 
    **/
    //---------------------------------------------------------------------
    public void setAssociatedStateIds(Vector value)
    {
        associatedStateIds = value;
    }

    //---------------------------------------------------------------------
    /**
        Retrieves the associated state Names. <P>
        @return associatedStateNames as HashMap
        @deprecated in version 13.3; not used 
    **/
    //---------------------------------------------------------------------
    public Vector getAssociatedStateNames()
    {
        return(associatedStateNames);
    }

    //---------------------------------------------------------------------
    /**
        Sets the associated state Names.  <P>
        @param value as HashMap
        @deprecated in version 13.3; not used 
    **/
    //---------------------------------------------------------------------
    public void setAssociatedStateNames(Vector value)
    {
        associatedStateNames = value;
    }
    
    //---------------------------------------------------------------------
    /**
        Retrieves the serial number. <P>
    ...
    **/
    ...
    //----------------------------------------------------------------------------
    /**
        Retrieves serialNumberRequired. <P>
    ...
    **/
    ...
    //------------------------------------------------------------------------
    /**
       Gets the taxRate property (BigDecimal) value.
       @return The taxRate property value.
    ...
    */
    ...
    //------------------------------------------------------------------------
    /**
       Sets the taxRate property (BigDecimal) value.
       @param value The new value for the property.
       @see #getTaxRate()
       @deprecated in 13.3
    */
    ...
    /**
       ...
       @param date the gift card expiration date
       @deprecated in 13.3
    */
    //------------------------------------------------------------------------
    public void setGiftCardExpirationDate(EYSDate date)
    {
        this.giftCardExpirationDate = date;
    }

    //------------------------------------------------------------------------
    /**
       Gets the gift card expiration date description.
       @return the gift card expiration date
       @deprecated in 13.3
    */
    //------------------------------------------------------------------------
 ...
 */   
    /**
     * @param isItemFromRetrievedTransaction The isItemFromRetrievedTransaction to set.
       @deprecated in 13.3
     */
/* TODO remove comment    public void setItemFromRetrievedTransaction(boolean isItemFromRetrievedTransaction)
    {
        this.isItemFromRetrievedTransaction = isItemFromRetrievedTransaction;
    }
*/
}
