 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/ado/journal/TransactionFormatter.java /rgbustores_13.4x_generic_branch/5 2011/09/07 09:31:36 cgreene Exp $
 * ===========================================================================
 * NOTES
...
public class TransactionFormatter implements RegisterJournalFormatterIfc
{
        ...
        StringBuffer sb)
    {
        ...
        if (txnMap.get(JournalConstants.ISSUED_STORE_CREDITS) != null)
        {
            ...
            for (Iterator iter = storeCredits.iterator(); iter.hasNext();)
            {
				...
				{
					dataArgs = new Object[]{(String) storeCreditMap.get(TenderConstants.ENTRY_METHOD)};
        ...
        StringBuffer sb)
    {
        ...
        if (irsCustomer != null)
        {
			...
			sb.append(" ");
            Vector<AddressIfc> addresses = irsCustomer.getAddresses();
            if (addresses.size() > 0)
            {
                AddressIfc address = addresses.firstElement();
    ...
    {
		change = change.negate(); // This should be a negative for journalling purposes
		dataArgs = new Object[]{change.toGroupFormattedString()};
		sb.append(Util.EOL)
		.append(I18NHelper.getString(I18NConstantsIfc.EJOURNAL_TYPE, JournalConstantsIfc.CHANGE_DUE_LABEL, dataArgs,journalLocale));
     ...
     * @param numSpaces
     * @param change Amount left as change
     * @deprecated As of release 12.0, replaced by {@link #printChangeDue(StringBuffer,CurrencyIfc)}
     */
    protected void printChangeDue(StringBuffer sb, int numSpaces, CurrencyIfc change)
    {
        printChangeDue(sb, change);
    }
}
