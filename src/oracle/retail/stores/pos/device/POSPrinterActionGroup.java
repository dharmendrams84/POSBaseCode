 ...
 * ===========================================================================
 * $Header: rgbustores/applications/pos/src/oracle/retail/stores/pos/device/POSPrinterActionGroup.java /main/29 2011/02/27 20:37:36 cgreene Exp $
 * ===========================================================================
 * NOTES
    ...
    implements POSPrinterActionGroupIfc
{
     ...
     */
    protected Map<String, Integer> frankingLines = new HashMap<String, Integer>();
    ...
    /**
     * Closes printer after timeout errort
     * <P>
    ...
    protected ReceiptPrinterIfc getReceiptPrinter(EYSPrintableDocumentIfc document, boolean isCleanReceipt) throws DeviceException
    {
        ...
        else
        {
            ...
            {
                fopReceiptPrinter = (FoReceiptPrinter)BeanLocator.getDeviceBean(FoReceiptPrinter.BEAN_KEY_ERECEIPT);
            }
            ...
            fopReceiptPrinter.setXmlDocument(getXMLTemplate(blueprintName, this.eReceiptDocTypeXMLMapping));
            fopReceiptPrinter.setFopTemplate(getFOPTemplate(blueprintName, this.eReceiptDocTypeFOPMapping));
    ...
    public void printNormal(int station, String data) throws DeviceException
    {
        ...
        // override slip line size, if necessary
        try
        {
            if (station == POSPrinterConst.PTR_S_SLIP && getSlipLineSize() != USE_SLIP_LINE_SIZE_DEFAULT)
            {
                getPOSPrinter().setSlpLineChars(getSlipLineSize());
            }
        }
        // if exception occurs, hope for the best and move on
        // When running simulated devices, printer is not open, so this
        // exception is thrown.
        catch (JposException e)
        {
            logger.warn("POSPrinter Exception: Cannot set slip line characters: " + e.getMessage() + " Error Code:  "
                    + Integer.toString(e.getErrorCode()) + "  xtended Error Code:  "
